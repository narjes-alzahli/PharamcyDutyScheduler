...
def show_input_page():
    ...
    with col2:
        ...
        time_off_file = st.file_uploader(
            "Leave CSV",
            type=["csv"],
            help="employee,date,code"
        )
        
        locks_file = st.file_uploader(
            "Special Requirements CSV",
            type=["csv"], 
            help="employee,date,shift,force"
        )
...
def show_reports_page():
    ...
    if schedule_df is not None:
        ...
        fig.update_layout(
            title="Employee Schedule Heatmap",
            xaxis_title="Date",
    ...
    if coverage_df is not None:
        ...
        shift_coverage = coverage_df.groupby("shift").agg({
            "needed": "sum",
            "assigned": "sum",
            "shortfall": "sum"
        }).reset_index()
        ...
        daily_coverage = coverage_df.groupby("date").agg({
            "shortfall": "sum"
        }).reset_index()
...
def load_sample_data():
    ...
    # Sample time off
    time_off_data = [
        {"employee": "Rasha", "date": date(2025, 3, 5), "code": "CL"},
        {"employee": "Ameera", "date": date(2025, 3, 10), "code": "W"}
    ]
    st.session_state["time_off_df"] = pd.DataFrame(time_off_data)
    
    # Sample locks
    locks_data = [
        {"employee": "Ameera", "date": date(2025, 3, 12), "shift": "APP", "force": 1},
        {"employee": "Shatha", "date": date(2025, 3, 14), "shift": "N", "force": 0}
    ]
